...
def q_pred(s, dt):
    ...
    r = s[3]; B = s[4]; omega = s[5]; gamma = s[6]; w1 = s[7]; w2 = s[8]
    vel = 5 * r * omega
...
def estRun(time, dt, internalStateIn, steeringAngle, pedalSpeed, measurement):
    ...
    sig_vv = np.array([[radius_var, 0, 0, 0], [0, baseline_var, 0, 0], [0, 0, .5, 0], [0, 0, 0, .5]])
    ...
    ksi = np.array([x,y,theta, radius_m, baseline_m, omega, gamma, mean_w[0], mean_w[1]]) # NOTE(JH) the last two term should contain mean_w??
