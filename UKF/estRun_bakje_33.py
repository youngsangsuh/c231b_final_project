...
def q_pred(s, dt):
    ...
    r = s[3]; B = s[4]; omega = s[5]; gamma = s[6], 
    vel = 5 * r * omega
...
def h(s, mean_w):
    ...
    r = s[3]; B = s[4]; 
    s_z = np.zeros((2,))
    s_z[0] = x + 1/2 * B * np.cos(theta) + mean_w[0]
    s_z[1] = y + 1/2 * B * np.sin(theta) + mean_w[1]
...
def estRun(time, dt, internalStateIn, steeringAngle, pedalSpeed, measurement):
    ...
    sig_vv = np.array([[radius_var, 0, 0, 0], [0, baseline_var, 0, 0], [0, 0, 0.75, 0], [0, 0, 0, 0.75]])
    ...
    ksi = np.array([x,y,theta, radius_m, baseline_m, omega, gamma]) # NOTE(JH) the last two term should contain mean_w??
    ...
    # P_ksi = block_diag(P_m, sig_vv)

    # L_ksi = cholesky(P_ksi, lower=True)
    S, T = sp.linalg.eig(P_m)
    L_ksi_1 = T.T @ np.diag(np.sqrt(np.real(S)))

    S, T = sp.linalg.eig(sig_vv)
    L_ksi_2 = T.T @ np.diag(np.sqrt(np.real(S)))

    L_ksi = block_diag(L_ksi_1, L_ksi_2)

    for i in range(n_ksi):
        sigma_ksi[:,2*i] = ksi + L_ksi[:,i]
        sigma_ksi[:,2*i+1] = ksi - L_ksi[:,i]

    ### Prior update
    sigma_ksi_p = np.zeros((n_ksi, n_points)) ## 9,19
    for i in range(n_points):
        sigma_ksi_p[:,i] = q_pred(sigma_ksi[:,i], dt) ############################### 

    Xp_hat = np.mean(sigma_ksi[0:3,:], axis = 1)
    ...
    if not (np.isnan(measurement[0]) or np.isnan(measurement[1])):
        ...
        for i in range (n_points):
            sigma_z[:,i] = h(sigma_ksi_p[:,i], mean_w) ###################################

        z_hat = np.mean(sigma_z, axis = 1)
